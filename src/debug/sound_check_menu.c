...
static void Task_InitSoundCheckMenu_CreateWindows(u8 taskId) // SanitizeDayCareMailForRuby
{
    ...
    if (!gPaletteFade.active)
    {
        ...
        AddTextPrinterParameterized(WIN_INFO, 2, driverStr, 108, 14, TEXT_SKIP_DRAW, NULL);
...
static bool8 Task_ProcessSoundCheckMenuInput(u8 taskId) // sub_080E8688
{
    ...
    else if (JOY_NEW(A_BUTTON))
    {
        if (gTasks[taskId].tWindowSelected != TEST_MUS)
        {
            if (gTasks[taskId].tSeIndexOld != 0)
            {
                ...
                {
                    m4aSongNumStopGbs(gTasks[taskId].tSeIndexOld, gTasks[taskId].tGBSounds);
                    m4aSongNumStartGbs(gTasks[taskId].tSeIndex, gTasks[taskId].tGBSounds);
        ...
        else
        {
            if (gTasks[taskId].tBgmIndexOld != 0)
            {
                ...
                {
                    m4aSongNumStopGbs(gTasks[taskId].tBgmIndexOld + (MUS_LITTLEROOT_TEST - 1), gTasks[taskId].tGBSounds);
                    m4aSongNumStartGbs(gTasks[taskId].tBgmIndex + (MUS_LITTLEROOT_TEST - 1), gTasks[taskId].tGBSounds);
    ...
    {
        m4aSongNumStartGbs(SE_SELECT, FALSE);
    ...
    else if (JOY_NEW(START_BUTTON))
    {
        ...
        {
            m4aSongNumStopGbs(gTasks[taskId].tBgmIndex + (MUS_LITTLEROOT_TEST - 1), !gTasks[taskId].tGBSounds);
            m4aSongNumStartGbs(gTasks[taskId].tBgmIndex + (MUS_LITTLEROOT_TEST - 1), gTasks[taskId].tGBSounds);
        }
    }
