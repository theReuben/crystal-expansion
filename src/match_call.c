...
static const struct massOutbreakPhoneCallData outbreakNone_unused = {
...
void RedrawMatchCallTextBoxBorder(void)
{
    ...
    u32 tileNum = GetBgAttribute(bg, BG_ATTR_BASETILE);
    u32 paletteId = 14;
...
// CrystalDust's name for the same routine; its phone-call task calls this.
bool32 ExecuteMatchCallTextPrinter(int windowId)
{
    return RunMatchCallTextPrinter(windowId);
}
...
void SelectMatchCallMessage_Hangup(int trainerId, u8 *str, bool8 isCallingPlayer, const struct PhoneContact *phoneContact)
{
	 u32 matchCallId;
	 u32 rematchId;
	 const struct MatchCallText *matchCallText = NULL;
...
void SelectMatchCallMessage_Opening(int trainerId, u8 *str, bool8 isCallingPlayer, const struct PhoneContact *phoneContact)
{
	 ...
	 if(isCallingPlayer)
	 {
		 switch(timeOfDay){
			 ...
			 case TIME_EVENING: // CrystalDust has no evening text; GSC switched at dusk (D20a)
			case TIME_NIGHT:
...
bool32 SelectMatchCallMessage(int trainerId, u8 *str, bool8 isCallingPlayer, const struct PhoneContact *phoneContact)
{
    ...
    else if (gMatchCallTrainers[matchCallId].outbreakData.species != SPECIES_NONE && gSaveBlock1Ptr->outbreakPokemonSpecies == gMatchCallTrainers[matchCallId].outbreakData.species)
    {
    	matchCallText = &gMatchCallTrainers[matchCallId].remindoutbreakText;
    		...
    		((!isCallingPlayer && CanMatchCallIdAcceptRematch(matchCallId, GetDayOfWeek(), GetTimeOfDay())) ||
    ...
    if (gWildMonHeaders[i].mapGroup != MAP_GROUP(MAP_UNDEFINED)) // ??? This check is nonsense.
    {
        ...
        if (gWildMonHeaders[i].mapGroup != MAP_GROUP(MAP_UNDEFINED))
        {
            if (gWildMonHeaders[i].encounterTypes[timeOfDay].landMonsInfo)
            {
                ...
                species[0] = gWildMonHeaders[i].encounterTypes[timeOfDay].landMonsInfo->wildPokemon[slot].species; // One of the last four slots
                species[1] = gWildMonHeaders[i].encounterTypes[timeOfDay].landMonsInfo->wildPokemon[0].species;    // Most common slot
                ...
                StringCopy(gStringVar1, GetSpeciesName(species[0]));
                return TRUE;
            }
        }
    }
...
void BufferPokedexRatingForMatchCall(u8 *destStr)
{
    ...
    str = StringCopy(buffer, gPokedexRatingText_AreYouCurious);
    *(str++) = CHAR_PROMPT_CLEAR;
    str = StringCopy(str, gPokedexRatingText_SoYouveSeenAndCaught);
    *(str++) = CHAR_PROMPT_CLEAR;
    ...
    if (IsNationalPokedexEnabled())
    {
        *(str++) = CHAR_PROMPT_CLEAR;
        ...
        StringExpandPlaceholders(str, gPokedexRatingText_OnANationwideBasis);
    }
}
