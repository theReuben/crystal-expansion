...
enum TrainerPicID PlayerGenderToFrontTrainerPicId(enum Gender playerGender)
{
    if (playerGender != MALE)
        return FacilityClassToPicIndex(IS_FRLG ? FACILITY_CLASS_LEAF : FACILITY_CLASS_KRIS); // CrystalDust (D59)
    else
        return FacilityClassToPicIndex(IS_FRLG ? FACILITY_CLASS_RED : FACILITY_CLASS_GOLD); // CrystalDust (D59)
}
