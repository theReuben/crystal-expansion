...
{
    // Gold and Kris share one house in New Bark Town, so there is no gendered
    // second house to check (D69).
    if (gSaveBlock1Ptr->location.mapGroup != MAP_GROUP(MAP_NEW_BARK_TOWN_PLAYERS_HOUSE_1F)
     || gSaveBlock1Ptr->location.mapNum != MAP_NUM(MAP_NEW_BARK_TOWN_PLAYERS_HOUSE_1F))
...
static void DoTVShowTodaysRivalTrainer(void)
{
    ...
    case 0:
        ...
        case MAPSEC_DYNAMIC:
            ...
            {
            // S.S. Tidal layouts are Hoenn content, absent from layouts.json. See D24.
            // sTVShowState = 10;                break;
            default:
                sTVShowState = 9;
                break;
            }
